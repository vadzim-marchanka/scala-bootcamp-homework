...
class CollectionsSpec extends AnyFlatSpec with ScalaCheckDrivenPropertyChecks {
  ...
  "scanLeft" should "work correctly on empty list" in {
  ...
  "count" should "pass on empty string" in {
    count("") shouldEqual List()
  }

  "count" should "pass if the string consist of one letter" in {
    count("aaaa") shouldEqual List(('a', 4))
  }
}
